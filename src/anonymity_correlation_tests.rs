...
#[test]
fn correlation_single_user_below_random_chance() {
    let r = run_simulation(1, 20_000);
    assert!(
        r.abs() <= CORRELATION_THRESHOLD,
        "single-user correlation too high: {r}"
    );
}

#[test]
fn correlation_multi_user_below_random_chance() {
    let r = run_simulation(5, 100_000);
    assert!(
        r.abs() <= CORRELATION_THRESHOLD,
        "multi-user correlation too high: {r}"
    );
}
